...
async function main() {
  try {
    ...
    await prisma.frozenStudent.deleteMany();
    ...
    const validSeatMatrices = seatMatrices.filter((seat) => {
      if (!departmentIds.has(seat.departmentId)) {
        console.warn(
          `Warning: Invalid department ID found in seat matrix: ${seat.departmentId}`
        );
        return false;
      }
      return true;
    ...
    if (validSeatMatrices.length !== seatMatrices.length) {
      console.warn(
        `Found ${
          seatMatrices.length - validSeatMatrices.length
        } invalid seat matrix entries`
      );
    }
    ...
    const validApplications = studentApplications.filter((student) => {
      if (!student.applicationNumber) {
        console.warn(
          `Warning: Missing applicationNumber for student: ${student.studentName}`
        );
        return false;
      }
      return true;
    ...
    if (validApplications.length !== studentApplications.length) {
      console.warn(
        `Found ${
          studentApplications.length - validApplications.length
        } invalid student applications`
      );
    }
