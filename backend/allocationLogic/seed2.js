...
async function main() {
  try {
    ...
    await prisma.round2input.createMany({
      data: validApplications.map((student) => ({
        studentId: student.ApplicationNumber,
        departmentId: student.AllocatedCourse?.trim() || '',
        allocationRound: 2,
        category: student.StudentCategory || '',
        subCategory: student.StudentSubCategory || null,
        choiceNumber: parseInt(student.ChoiceNumber) || 0,
        jeeRank: parseInt(student.JEE_CRL) || 0,
        status: student.Status || 'FLOAT',
        feesPaid: student['Fee Paid'] === 'YES'
      }))
    });
