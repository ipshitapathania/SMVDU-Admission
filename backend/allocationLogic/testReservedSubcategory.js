...
export async function main(round) {
  ...
  console.log(
    `\n=== Starting Reserved Subcategory Allocation Test (Round ${round}) ===`
  );
  try {
    ...
    console.log(
      `Starting reserved subcategory allocation with ${students.length} students`
    );

    // Run allocation (typically round 3+)
    const results = await runReservedSubcategoryAllocation(students, round);
    ...
    const reportPath = path.join(
      "./",
      `reserved_subcategory_results_${new Date()
        .toISOString()
        .slice(0, 10)}.csv`
    );
