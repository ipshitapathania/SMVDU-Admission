...
async function main() {
    try {
        const round = 2; // Typically run after initial allocation round
        ...
        results.allocated.forEach(allocation => {
            console.log(`- ${allocation.studentId}: ${allocation.departmentId} ` +
                      `(${allocation.category}-${allocation.subCategory}) ` +
                      `Choice ${allocation.choiceNumber}`);
        });

        console.log('\nUpgraded Students:');
        results.allocated.filter(a => a.choiceNumber).forEach(allocation => {
            const original = results.vacated.find(v => v.studentId === allocation.studentId);
            if (original) {
                console.log(`- ${allocation.studentId}: ` +
        ...
        results.failures.forEach(failure => {
            if (failure.student) {
                console.log(`- Student ${failure.student}: ${failure.details || failure.error}`);
            } else {
...
async function verifyAllocationQuality(results) {
    ...
    allocations.reduce((prev, curr) => {
        if (prev && curr.jeeRank < prev.jeeRank) {
            console.warn(`Rank violation: ${curr.studentId} (CRL:${curr.jeeRank}) has seat while ` +
    ...
    results.allocated.forEach(newAlloc => {
        const oldAlloc = results.vacated.find(v => v.studentId === newAlloc.studentId);
        if (oldAlloc) {
            ...
            if (newPrefIndex >= oldPrefIndex) {
                console.warn(`Invalid upgrade: ${newAlloc.studentId} moved to ` +
    ...
    console.log(`Verification complete. ${rankingViolations} ranking violations, ${invalidUpgrades} invalid upgrades found.`);
}
